...
public class PublishEventProcessor extends CommonEventProcessor {
    ...
    public void onPermissionCheck(PermissionCheckEvent event) {
        ...
        hasPermission = MessageCarryType.lookup(source.getType()).equals(MessageCarryType.PUBLISH) || MessageCarryType.lookup(source.getType()).equals(MessageCarryType.PUBLISH_SUBSCRIBE);
        if (!hasPermission) {
    ...
    public void onPublish(PublishEvent event) {
        ...
        try {
            for (Message msg : context.getMessages()) {
                ...
                ConfigManager.Source s = context.getSource();
                ProxyProducer.produce(Constants.PROXY_EXCHANGE_NAME,
                        ...
                        properties);
                if(s.getSinks() != null ){
                    for(ConfigManager.Sink sink : s.getSinks()){
                        ProxyProducer.produce(Constants.PROXY_EXCHANGE_NAME,
                                context.getChannel(),
                                sink.getRoutingKey(),
                                msg.getContent(),
